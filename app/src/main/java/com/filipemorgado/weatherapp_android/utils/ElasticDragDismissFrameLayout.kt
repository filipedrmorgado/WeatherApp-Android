...
class ElasticDragDismissFrameLayout @JvmOverloads constructor(
        ...
        if (abs(totalDrag) >= dragDismissDistance) {
            ...
            if (mLastActionEvent == MotionEvent.ACTION_DOWN) {
                ...
                animate()
                    ...
                    .scaleY(1f)
                    .setDuration(200L)
                    .setInterpolator(AppUtils.getFastOutSlowInInterpolator(context))
                    .setListener(null)
                    .start()
            }
            totalDrag = 0f
    ...
    class SystemChromeFader(private val activity: Activity) :
        ...
        private val fadeNavBar: Boolean = AppUtils.isNavBarOnBottom(activity)
        ...
        override fun onDrag(
            ...
            if (elasticOffsetPixels > 0) {
                ...
                activity.window.statusBarColor = AppUtils.modifyAlpha(activity.window.statusBarColor, ((1f - rawOffset) * statusBarAlpha).toInt())
            } else if (elasticOffsetPixels == 0f) {
                // reset
                activity.window.statusBarColor = AppUtils.modifyAlpha(activity.window.statusBarColor, statusBarAlpha)
                activity.window.navigationBarColor = AppUtils.modifyAlpha(activity.window.navigationBarColor, navBarAlpha)
            } else if (fadeNavBar) {
                // dragging upward, fade the appbar_menu bar in proportion
                activity.window.navigationBarColor = AppUtils.modifyAlpha(activity.window.navigationBarColor,((1f - rawOffset) * navBarAlpha).toInt())
            }
        }
